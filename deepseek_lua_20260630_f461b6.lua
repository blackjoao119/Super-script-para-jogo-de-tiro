--[[
    ╔══════════════════════════════════════════════════════════════════════╗
    ║     WARCORE v2.0 - UI PERSONALIZADA (SEM RAYFIELD)                ║
    ║     Design Moderno • Responsivo • Mobile-Friendly                  ║
...
-- ============================================================
-- 3. MÓDULO DE COMBATE (Aim Assist + Trigger Bot + FOV Circle)
-- ============================================================
local Combat = {}
local fovCircle = nil
local aimTarget = nil
...
-- ============================================================
-- 4. MÓDULO VISUAL (ESP, Dot, Line, Micro-HUD)
-- ============================================================
local Visual = {}
...
-- ============================================================
-- 6. MÓDULO DE TELEPORTE (GUI integrada)
-- ============================================================
local Teleport = {}
Teleport.Locations = {}
...
-- Função para criar um botão com hover animado
local function createStyledButton(parent, text, size, backgroundColor, textColor, callback)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Primary, TextColor3 = Color3.fromRGB(0,0,0)}):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = backgroundColor or Theme.SurfaceLight, TextColor3 = textColor or Theme.Text}):Play()
    end)
...
local function createToggle(parent, labelText, initialValue, callback)
    ...
    indicator.Position = initialValue and UDim2.new(1, -26, 0.5, -11) or UDim2.new(0, 4, 0.5, -11)
    ...
    local function updateToggle(value)
        currentValue = value
        TweenService:Create(toggleBtn, TweenInfo.new(0.15), {BackgroundColor3 = value and Theme.Primary or Theme.SurfaceLight}):Play()
        TweenService:Create(indicator, TweenInfo.new(0.15), {Position = value and UDim2.new(1, -26, 0.5, -11) or UDim2.new(0, 4, 0.5, -11)}):Play()
...
local function createDropdown(parent, labelText, options, defaultOption, callback)
    ...
    dropdownBtn.Size = UDim2.new(0.5, -10, 1, -10)
    ...
    local function updateList()
        ...
        listFrame.Size = UDim2.new(0.5, -10, 0, #options * 32 + 4)
    end
    updateList()
...
-- Função para criar um color picker simplificado
local function createColorPicker(parent, labelText, initialColor, callback)
    ...
    colorBtn.Position = UDim2.new(1, -50, 0.5, -15)
    ...
    colorBtn.MouseButton1Click:Connect(function()
        -- Simples: abre um input para o usuário digitar valores RGB (ou usar uma paleta simples)
        -- Para simplificar, vamos permitir escolher entre algumas cores predefinidas
        local colors = {
        ...
        -- Cria uma pequena paleta flutuante
        local palette = Instance.new("Frame", ScreenGui)
        palette.Size = UDim2.new(0, 200, 0, 50)
        palette.Position = UDim2.new(0.5, -100, 0.5, -25)
...
-- ============================================================
-- 9. ESTRUTURA DO MENU PRINCIPAL
-- ============================================================
local MainFrame = Instance.new("Frame", ScreenGui)
MainFrame.Size = UDim2.new(0, 0, 0, 0) -- começa fechado
...
FloatingBtn.Position = UDim2.new(0.9, -70, 0.9, -70)
...
function CreateTab(name, icon)
    ...
    content.Size = UDim2.new(1, 0, 0, 0) -- será ajustado dinamicamente
...
local LightTab = CreateTab("Iluminação", "💡")
...
-- Switch para primeira aba
SwitchTab("Combate")

-- ============================================================
-- 10. PREENCHIMENTO DAS ABAS
-- ============================================================
...
local aimToggle = createToggle(CombatTab, "Ativar Mira Assistida", Config.Settings.AimAssist, function(v) Config.Settings.AimAssist = v end)
...
-- Visual
AddSection(VisualTab, "Elementos de Rastreamento")
...
createToggle(VisualTab, "Micro-HUD Distância", Config.Settings.MicroDistEnabled, function(v) Config.Settings.MicroDistEnabled = v end)
...
-- Opções RX
AddSection(RxTab, "Configurações Avançadas")
...
AddSection(RxTab, "Personalização de Mira")
createColorPicker(RxTab, "Cor da Linha", Config.Settings.LineColor, function(v) Config.Settings.LineColor = v end)
createSlider(RxTab, "Espessura da Linha", 0.5, 5, 0.5, Config.Settings.LineThickness, function(v) Config.Settings.LineThickness = v end)
createColorPicker(RxTab, "Cor da Distância", Config.Settings.DistColor, function(v) Config.Settings.DistColor = v end)
...
createToggle(MoveTab, "Sem Dano de Queda", Config.Settings.NoFallDamage, function(v) Config.Settings.NoFallDamage = v end)

-- Teleporte (GUI simplificada)
AddSection(TeleportTab, "Locais Salvos")
-- Botão para salvar posição atual
local saveBtn = createStyledButton(TeleportTab, "💾 Salvar Local Atual", UDim2.new(1, 0, 0, 40), Theme.Success, Color3.fromRGB(0,0,0), function()
    local char = GetCharacter()
    if not char or not GetRootPart(char) then return end
    local pos = GetRootPart(char).Position
    local name = tostring(#Teleport.Locations + 1)
    table.insert(Teleport.Locations, {name = "Local " .. name, x = pos.X, y = pos.Y, z = pos.Z})
    RefreshTeleportList()
end)
...
Instance.new("UICorner", teleportList).CornerRadius = UDim.new(0, 8)
...
function RefreshTeleportList()
    ...
    for _, loc in ipairs(Teleport.FixedLocations) do
        local btn = createStyledButton(teleportList, loc.name .. " (fixo)", UDim2.new(1, 0, 0, 30), Theme.Warning, Color3.fromRGB(0,0,0), function()
    ...
    for i, loc in ipairs(Teleport.Locations) do
        ...
        local btn = createStyledButton(frame, loc.name, UDim2.new(1, -50, 1, 0), Theme.SurfaceLight, Theme.Text, function()
            local root = GetRootPart(GetCharacter())
            if root then
                root.CFrame = CFrame.new(loc.x, loc.y, loc.z)
            end
    ...
    teleportList.CanvasSize = UDim2.new(0, 0, 0, total * 34 + 10)
end
RefreshTeleportList()
...
createStyledButton(ExtraTab, "🔄 Rejoin", UDim2.new(1, 0, 0, 40), Theme.Primary, Color3.fromRGB(0,0,0), function()
    game:GetService("TeleportService"):Teleport(game.PlaceId)
end)
createStyledButton(ExtraTab, "💥 Explodir", UDim2.new(1, 0, 0, 40), Theme.Danger, Color3.fromRGB(255,255,255), function()
    local char = GetCharacter()
...
-- Notificação inicial (usando um simples popup)
...
notify.Text = "👑 WARCORE v2.0 carregado!"
notify.TextColor3 = Theme.Primary
notify.Font = Enum.Font.GothamBold
notify.TextSize = 18
notify.ZIndex = 15
Instance.new("UICorner", notify).CornerRadius = UDim.new(0, 8)
TweenService:Create(notify, TweenInfo.new(0.3), {Position = UDim2.new(0.5, -150, 0.15, 0)}):Play()
task.wait(3)
TweenService:Create(notify, TweenInfo.new(0.3), {Position = UDim2.new(0.5, -150, -0.1, 0)}):Play()
task.wait(0.3)
notify:Destroy()

print("WARCORE v2.0 - UI Personalizada carregada com sucesso!")