...
--// =============================================
--// 🔄 ABA TELEPORTE (NATIVA RAYFIELD)
--// =============================================
local savedLocations = {}  -- {name, x, y, z}

-- Seção: Localização Atual
TeleportTab:CreateSection("📍 Minha Localização Atual")

local currentPosParagraph = TeleportTab:CreateParagraph({
    Title = "Coordenadas",
    Content = "Aguardando..."
})

-- Atualiza a posição periodicamente (substitui o parágrafo)
task.spawn(function()
    while true do
        local char = Player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            -- Recria o parágrafo para atualizar (Rayfield não permite editar parágrafo diretamente em algumas versões)
            pcall(function() currentPosParagraph:Destroy() end)
            currentPosParagraph = TeleportTab:CreateParagraph({
                Title = "Coordenadas",
                Content = text
            })
        end
...
local nameInput = TeleportTab:CreateInput({
    Name = "Nome do Local",
    PlaceholderText = "Ex: Spawn, Farm, Boss...",
    Callback = function(text) end -- o valor será capturado no botão
})

local savedName = ""
-- Como o Input não expõe o texto diretamente, usaremos uma variável auxiliar
nameInput.OnTextChanged = function(text) -- hack: conectamos diretamente no objeto retornado
    savedName = text
end
-- Caso a linha acima não funcione, usamos o callback nativo:
-- No Callback definido no CreateInput já poderíamos setar, mas o Rayfield pode não passar o valor. Vamos usar um truque:
local tempInputConnection
tempInputConnection = nameInput.InputTextChanged or nameInput.TextChanged -- depende da versão
if not tempInputConnection then
    -- Alternativa: usar o retorno do CreateInput que pode ter um método :GetText()
    -- Mas não é padrão. Para garantir, faremos um loop que lê o texto do objeto GUI.
    -- Entretanto, a Rayfield do sirius.menu retorna uma tabela com o objeto TextBox interno em .Instance.
    if nameInput.Instance and nameInput.Instance:IsA("TextBox") then
        nameInput.Instance:GetPropertyChangedSignal("Text"):Connect(function()
            savedName = nameInput.Instance.Text
        end)
    end
else
    tempInputConnection:Connect(function(text)
        savedName = text
    end)
end
...
TeleportTab:CreateButton({
    ...
    Callback = function()
        ...
        if savedName == "" then
            Rayfield:Notify({
                Title = "Aviso",
                Content = "Digite um nome para o local.",
                Duration = 2,
                Image = 4483362458
            })
            return
        ...
        -- Limpa o campo de nome (opcional, mas não podemos resetar o Input facilmente)
        -- Podemos limpar via nameInput.Instance.Text = "" se disponível
        if nameInput.Instance and nameInput.Instance:IsA("TextBox") then
            nameInput.Instance.Text = ""
        end
        savedName = ""
        updateLocationDropdown()
        Rayfield:Notify({
            Title = "Salvo!",
            Content = "Local '" .. savedLocations[#savedLocations].name .. "' adicionado.",
            Duration = 2,
            Image = 4483362458
        })
    end
...
-- Seção: Locais Salvos
TeleportTab:CreateSection("📂 Locais Salvos")
...
local function updateLocationDropdown()
    local names = {}
    for _, loc in ipairs(savedLocations) do
        table.insert(names, loc.name)
    end
    locationDropdown:Set(names)
end

updateLocationDropdown()

TeleportTab:CreateButton({
    Name = "🚀 Teleportar para Local Selecionado",
    Callback = function()
        local selected = locationDropdown.CurrentOption
        if not selected or selected == "" then
            Rayfield:Notify({
                Title = "Erro",
                Content = "Nenhum local selecionado.",
                Duration = 2,
                Image = 4483362458
            })
            return
        ...
        if target then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                Rayfield:Notify({
                    Title = "Teleportado!",
                    Content = "Movido para " .. target.name,
                    Duration = 2,
                    Image = 4483362458
                })
            end
...
TeleportTab:CreateButton({
    Name = "🗑 Excluir Local Selecionado",
    Callback = function()
        local selected = locationDropdown.CurrentOption
        if not selected or selected == "" then
            Rayfield:Notify({
...
Player.CharacterAdded:Connect(function()
    ...
    -- Atualiza a exibição de coordenadas e dropdown (a lista persiste)
    updateLocationDropdown()
end)

Rayfield:Notify({
    Title = "👑 WARCORE v1.2.0",
    Content = "Aba Teleporte nativa ativada, meu rei!",
    Duration = 5,
    Image = 4483362458
})