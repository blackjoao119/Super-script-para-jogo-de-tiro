...
--// =============================================
--// 🔥 SISTEMA DE TELEPORTE – CORRIGIDO E FUNCIONAL
--// =============================================
...
-- ========================
-- JANELA PRINCIPAL (PRECISA SER CRIADA ANTES DE REFERENCIÁ-LA NO CALLBACK)
-- ========================
...
-- Barra de título e elementos internos (mesmos do último script funcional)
...
-- Função toggleMenu (não mexe no botão flutuante)
...
-- Toggle principal (CRIADO ANTES DO BOTÃO)
local teleportToggle = TeleportTab:CreateToggle({
    Name = "Ativar Sistema de Teleporte",
    CurrentValue = false,
    Callback = function(value)
        -- Atualiza visual do botão flutuante
        if value then
            -- ON: grande e destaque
...
-- ========================
-- BOTÃO FLUTUANTE (SEMPRE VISÍVEL, CONTROLA O TOGGLE)
-- ========================
...
TpFloatingBtn.BackgroundColor3 = Color3.fromRGB(30, 33, 45)
TpFloatingBtn.BackgroundTransparency = 0.4
TpFloatingBtn.Text = "📍"
TpFloatingBtn.TextColor3 = Color3.fromRGB(0, 240, 255)
TpFloatingBtn.Font = Enum.Font.GothamBold
TpFloatingBtn.TextSize = 16
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                ...
                TweenService:Create(TpFloatingBtn, TweenInfo.new(0.1), {Size = UDim2.new(0, targetSize, 0, targetSize)}):Play()
            end
        end)
    end
end)
...
TpFloatingBtn.InputEnded:Connect(function(input)
    if not hasMoved and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
        teleportToggle:Set(not teleportToggle.CurrentValue)
    end
    hasMoved = false
end)
...
Rayfield:Notify({
    ...
    Content = "Sistema de teleporte 100% funcional!",
    Duration = 5,
    Image = 4483362458
})