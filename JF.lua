...
getgenv().SystemConfig = {
    ...
    -- NOVO: Cor da distância
    DistColor = Color3.fromRGB(255, 255, 255),
    -- NOVO: Forma do ponto
    DotShape = "●"  -- padrão círculo
}
...
-- NOVO: Configurações da Linha e Ponto
...
local function refreshContainer()
    -- Remove todos os elementos visuais antigos (exceto o UIListLayout)
    ...
    for i, loc in ipairs(FixedLocations) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if root then
                ...
                Rayfield:Notify({
                    Title = "✅ Teleportado",
                    Content = "Você foi para: " .. loc.name,
                    Duration = 2,
                    Image = 4483362458
                })
            end
...
clearBtn.MouseButton1Click:Connect(function()
    if #savedLocations == 0 then
        Rayfield:Notify({
            Title = "⚠️ Aviso",
            Content = "Nenhum local do usuário para limpar.",
            Duration = 2,
            Image = 4483362458
        })
        return
    ...
    Rayfield:Notify({
        Title = "🗑️ Limpo",
        Content = "Todos os seus locais foram removidos.",
        Duration = 2,
        Image = 4483362458
    })
end)
...
TpFloatingBtn.InputEnded:Connect(function(input)
    if isDragging and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        elseif hasMoved then
            -- Snap para posição válida (evita sair da tela)
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Player and p.Character then
            ...
            if head then
                ...
                if not dot then
                    -- NOVO: usar TextLabel para suportar formas
                    local bill = Instance.new("BillboardGui", head)
                    bill.Name = "System_Dot"
                    bill.Size = UDim2.new(0, 18, 0, 18) -- tamanho maior para acomodar caracteres
                ...
                if label then
                    label.TextColor3 = dotColor
                    label.Text = getgenv().SystemConfig.DotShape
                    -- Ajustar tamanho conforme forma
                    if getgenv().SystemConfig.DotShape == "●" then
                        label.TextSize = 16
                    elseif getgenv().SystemConfig.DotShape == "▲" then
                        label.TextSize = 18
                    elseif getgenv().SystemConfig.DotShape == "■" then
                        label.TextSize = 16
                    elseif getgenv().SystemConfig.DotShape == "◆" then
                ...
                if (getgenv().SystemConfig.MicroHpEnabled or getgenv().SystemConfig.MicroDistEnabled) and hum.Health > 0 then
                    ...
                    if currentHud then
                        ...
                        if getgenv().SystemConfig.MicroDistEnabled then
                            ...
                            -- NOVO: usar cor personalizada para distância
...
Rayfield:Notify({
    ...
    Content = "Novidades: Linha de Mira, cores customizáveis e formas!",
    Duration = 5,
    Image = 4483362458
})