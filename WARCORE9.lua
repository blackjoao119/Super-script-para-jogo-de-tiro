...
--// =============================================
--// ABA: MOVIMENTO (mantida como antes)
--// =============================================
...
MovimentTab:CreateToggle({
    ...
    CurrentValue = false,
    Callback = function(value)
        getgenv().SystemConfig.FlyInfinite = value
    end
})
...
MovimentTab:CreateSlider({
    ...
    CurrentValue = 50,
    Callback = function(value)
        getgenv().SystemConfig.FlySpeed = value
    end
})
...
MovimentTab:CreateToggle({
    ...
    CurrentValue = false,
    Callback = function(value)
        getgenv().SystemConfig.SpeedEnabled = value
        if value then
            local char = Player.Character
            if char then
                local hum = char:FindFirstChildOfClass("Humanoid")
                if hum then hum.WalkSpeed = getgenv().SystemConfig.SpeedValue end
            end
        else
            local char = Player.Character
            if char then
...
MovimentTab:CreateSlider({
    ...
    CurrentValue = 50,
    Callback = function(value)
        getgenv().SystemConfig.SpeedValue = value
...
MovimentTab:CreateToggle({
    ...
    CurrentValue = false,
    Callback = function(value)
        getgenv().SystemConfig.JumpEnabled = value
        local char = Player.Character
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
                if value then hum.JumpPower = getgenv().SystemConfig.JumpPower
...
MovimentTab:CreateSlider({
    ...
    CurrentValue = 100,
    Callback = function(value)
        getgenv().SystemConfig.JumpPower = value
...
MovimentTab:CreateToggle({
    ...
    CurrentValue = false,
    Callback = function(value)
        getgenv().SystemConfig.InfiniteJump = value
    end
})

--// =============================================
--// SISTEMA DE TELEPORTE – NOVO DESIGN COMPLETO
--// =============================================

-- Criar ScreenGui separada para o menu de teleporte (para gerenciar ZIndex e organização)
local tpGui = Instance.new("ScreenGui", CoreGui)
tpGui.Name = "WarcoreTeleporte"
tpGui.Enabled = true
tpGui.ResetOnSpawn = false

-- Botão Flutuante de Teleporte
local TpFloatingBtn = Instance.new("TextButton", tpGui)
TpFloatingBtn.Size = UDim2.new(0, 56, 0, 56)  -- Tamanho confortável para celular
TpFloatingBtn.Position = UDim2.new(0.8, 0, 0.7, 0)  -- Posição inicial (canto inferior direito)
TpFloatingBtn.BackgroundColor3 = Color3.fromRGB(11, 13, 23)
TpFloatingBtn.BackgroundTransparency = 0.1  -- Leve transparência
TpFloatingBtn.Text = "📍"  -- Ícone de teleporte
TpFloatingBtn.TextColor3 = Color3.fromRGB(0, 240, 255)
TpFloatingBtn.Font = Enum.Font.GothamBold
TpFloatingBtn.TextSize = 28
TpFloatingBtn.AutoButtonColor = false
TpFloatingBtn.Visible = false  -- Inicia invisível, controlado pelo Toggle
TpFloatingBtn.Active = true
TpFloatingBtn.ZIndex = 10  -- Fica acima da maioria dos elementos

-- Arredondar o botão
local floatCorner = Instance.new("UICorner", TpFloatingBtn)
floatCorner.CornerRadius = UDim.new(0, 20)  -- Bem arredondado

-- Sombra (simulada com Stroke mais escuro e grosso)
...
-- Função de animação suave para aparecer/desaparecer
local function animateButton(button, visible)
    if visible then
        button.Visible = true
        ...
        local tween = TweenService:Create(button, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
TpFloatingBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- Feedback tátil visual: leve aumento
        TweenService:Create(TpFloatingBtn, TweenInfo.new(0.1), {Size = UDim2.new(0, 62, 0, 62)}):Play()
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                dragging = false
                TweenService:Create(TpFloatingBtn, TweenInfo.new(0.1), {Size = UDim2.new(0, 56, 0, 56)}):Play()
            end
        end)
    end
end)
...
-- Clicar no botão flutuante abre a janela
TpFloatingBtn.MouseButton1Click:Connect(function()
    if not dragging then  -- Só abre se não estava arrastando (evitar abrir ao soltar após arraste)
        toggleMenu(true)
    end
end)

-- Janela principal do Teleporte (modernizada)
local tpFrame = Instance.new("Frame", tpGui)
tpFrame.Size = UDim2.new(0, 340, 0, 500)
tpFrame.Position = UDim2.new(0.5, -170, 0.5, -250)
tpFrame.BackgroundColor3 = Color3.fromRGB(14, 16, 26)
...
-- Sombra da janela
local frameShadow = Instance.new("UIStroke", tpFrame)
frameShadow.Thickness = 4
...
local titleBottom = Instance.new("Frame", titleBar)
titleBottom.Size = UDim2.new(1, 0, 0, 16)
titleBottom.Position = UDim2.new(0, 0, 0, 20)
titleBottom.BackgroundColor3 = Color3.fromRGB(18, 20, 30)
...
local winDragStart, winFrameStart
...
hoverEffect(saveBtn, Color3.fromRGB(0, 120, 220), Color3.fromRGB(0, 150, 255))

-- Container de scroll para locais salvos
...
clearBtn.Position = UDim2.new(0, 10, 0, 450)
...
hoverEffect(clearBtn, Color3.fromRGB(180, 40, 40), Color3.fromRGB(220, 60, 60))

-- Dados dos locais salvos (temporário)
...
local function refreshContainer()
    ...
    for i, loc in ipairs(savedLocations) do
        ...
        -- Efeito hover
        hoverEffect(btn, Color3.fromRGB(30, 35, 50), Color3.fromRGB(50, 55, 70))
...
-- Função para alternar visibilidade da janela com animação
function toggleMenu(visible)
    if visible then
        tpFrame.Visible = true
        tpFrame.Size = UDim2.new(0, 0, 0, 500)
        ...
        animateButton(TpFloatingBtn, false)
    else
        ...
        -- Mostrar botão flutuante, se o Toggle ainda estiver ativo
        if teleportToggle.CurrentValue then
            animateButton(TpFloatingBtn, true)
        end
    end
end
...
minimizeBtn.MouseButton1Click:Connect(function()
    toggleMenu(false)  -- Apenas minimiza, o botão reaparecerá
end)

closeBtn.MouseButton1Click:Connect(function()
    toggleMenu(false)
    teleportToggle:Set(false)  -- Desativa o Toggle, o que esconderá o botão via callback
end)

-- Toggle na Rayfield que controla tudo
local teleportToggle = TeleportTab:CreateToggle({
    Name = "Ativar Sistema de Teleporte",
    CurrentValue = false,
    Callback = function(value)
        if value then
            -- Ativou: mostrar botão flutuante, janela fechada
            animateButton(TpFloatingBtn, true)
        else
            -- Desativou: fechar janela e esconder botão
...
-- Atualizar posição no RenderStepped
...
-- Forçar Speed e Jump no Heartbeat (Persistência)
...
Rayfield:Notify({
    ...
    Content = "Sistema de teleporte reformulado com sucesso!",
    Duration = 5,
    Image = 4483362458
})