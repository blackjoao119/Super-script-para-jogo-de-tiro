...
--// =============================================
--// ABA: MOVIMENTO (REFEITA – FLY, SPEED, PULO)
--// =============================================
...
local function startFly()
    ...
    -- BodyGyro para estabilizar orientação (não rotacionar o personagem)
    ...
    -- Loop de controle
    FlyConnection = RunService.Stepped:Connect(function(time, delta)
        ...
        -- Manter orientação original (sem girar)
        FlyGyro.CFrame = root.CFrame * CFrame.Angles(0, 0, 0)
    end)
end

-- Toggle Fly
MovimentTab:CreateSection("🕊️ Fly (Voo Livre)")
local flyToggle = MovimentTab:CreateToggle({
    Name = "Ativar Fly",
    CurrentValue = false,
    Callback = function(value)
        getgenv().SystemConfig.FlyEnabled = value
        if value then
            startFly()
        else
            stopFly()
...
-- Lógica do Pulo Infinito no loop principal (adicionado abaixo)

--// =============================================
--// MENU FLUTUANTE DE TELEPORTE (já existente)
--// =============================================
...
Player.CharacterAdded:Connect(function()
    ...
    -- Restaurar Fly se ativo
    if getgenv().SystemConfig.FlyEnabled then
        startFly()
    end
    -- Restaurar Speed e Jump
    if getgenv().SystemConfig.SpeedEnabled then
    ...
    if getgenv().SystemConfig.JumpEnabled then
        local hum = Player.Character and Player.Character:FindFirstChildOfClass("Humanoid")
        if hum then hum.JumpPower = getgenv().SystemConfig.JumpPower end
    end
end)

Rayfield:Notify({
    Title = "👑 WARCORE v1.2.0",
    Content = "Movimento turbinado: Fly, Speed, Super Pulo e Pulo Infinito!",
    Duration = 5,
    Image = 4483362458
})