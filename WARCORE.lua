...
--// =============================================
--// 🔥 SISTEMA DE TELEPORTE – NOVA LÓGICA
--// =============================================
...
-- Função toggleMenu (controla apenas a janela)
...
-- Função para animar o botão flutuante (mostrar/ocultar)
...
-- Toggle principal (interruptor mestre)
local teleportToggle = TeleportTab:CreateToggle({
    ...
    Callback = function(value)
        if value then
            ...
            -- Esconder botão flutuante e fechar janela se aberta
...
-- ========================
-- BOTÃO FLUTUANTE (CONTROLE EXCLUSIVO DA JANELA)
-- ========================
local TpFloatingBtn = Instance.new("TextButton", tpGui)
TpFloatingBtn.Size = UDim2.new(0, 58, 0, 58)          -- tamanho padrão quando visível
...
TpFloatingBtn.Visible = false         -- inicia invisível (Toggle desligado)
...
-- Botões da janela (apenas fecham a janela, não desligam o sistema)
...
Rayfield:Notify({
    ...
    Content = "Novo sistema de teleporte independente ativado!",
    Duration = 5,
    Image = 4483362458
})