...
--// =============================================
--// ABA: TELEPORTE (VERSÃO COM NOMES)
--// =============================================
local savedPositions = {}  -- Lista de {name, pos} – reseta ao sair do jogo

local currentName = ""     -- Armazena o nome digitado no campo de texto

TeleportTab:CreateSection("Salvar Posição com Nome")

TeleportTab:CreateInput({
    Name = "Nome do local",
    PlaceholderText = "Ex: Base inimiga",
...
TeleportTab:CreateButton({
    ...
    Callback = function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            local name = (currentName ~= "" and currentName) or "Local " .. (#savedPositions + 1)
            table.insert(savedPositions, {name = name, pos = pos})
            updateSavedDropdown()
            Rayfield:Notify({
                Title = "Posição Salva",
                Content = name .. " | X:" .. math.floor(pos.X) .. " Y:" .. math.floor(pos.Y) .. " Z:" .. math.floor(pos.Z),
                Duration = 3,
                Image = 4483362458
            })
        end
...
local savedDropdown = TeleportTab:CreateDropdown({
...
local function updateSavedDropdown()
    local names = {}
    for _, entry in ipairs(savedPositions) do
        table.insert(names, entry.name)
    end
    savedDropdown:Set(names)
end
...
TeleportTab:CreateButton({
    ...
    Callback = function()
        local selectedName = savedDropdown.CurrentOption
...
TeleportTab:CreateButton({
    Name = "🗑️ Deletar Local Selecionado",
    Callback = function()
        local selectedName = savedDropdown.CurrentOption
        if selectedName and selectedName ~= "" then
            ...
            Rayfield:Notify({
                Title = "Removido",
                Content = "Local " .. selectedName .. " deletado.",
                Duration = 2,
                Image = 4483362458
            })
        end
...
TeleportTab:CreateButton({
    Name = "🗑️ Limpar Todos os Locais",
    Callback = function()
        savedPositions = {}
        updateSavedDropdown()
        Rayfield:Notify({
            Title = "Limpo",
            Content = "Todas as posições removidas.",
            Duration = 2,
            Image = 4483362458
        })
    end
...
TeleportTab:CreateSection("Teleporte para Jogador")
        ...
        if selectedName and selectedName ~= "" then
            ...
            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if char and char:FindFirstChild("HumanoidRootPart") then
                    ...
                    Rayfield:Notify({
                        Title = "Teleportado",
                        Content = "Você foi até " .. targetPlayer.Name,
                        Duration = 3,
                        Image = 4483362458
                    })
                end
...
Rayfield:Notify({
    ...
    Content = "Aba Teleporte com nomes ativada, meu rei!",
    Duration = 5,
    Image = 4483362458
})