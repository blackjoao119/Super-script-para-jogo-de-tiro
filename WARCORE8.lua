...
getgenv().SystemConfig = {
    ...
    FlyEnabled = false,
    FlySpeed = 50,
    FlyInfinite = false,        -- Modo infinito (câmera)
    SpeedEnabled = false,
    SpeedValue = 50,
    JumpEnabled = false,
    JumpPower = 100,
...
-- Valores originais do personagem (para restauração segura)
...
--// =============================================
--// ABA: MOVIMENTO (NOVO FLY INTEGRADO)
--// =============================================

-- Seção Fly
MovimentTab:CreateSection("🕊️ Fly")

-- Funções de controle do Fly
...
local function startFly()
    ...
    -- Loop de movimento (RenderStepped)
    ...
    flyConnection = RunService.RenderStepped:Connect(function()
        ...
        if getgenv().SystemConfig.FlyInfinite then
            -- Modo Infinito: voa sempre na direção da câmera
            targetVel = Camera.CFrame.LookVector * speed
        else
            -- Modo Normal: responde ao analógico (MoveDirection)
            local moveDir = hum.MoveDirection
            if moveDir.Magnitude > 0 then
                -- Converte a direção do mundo para a orientação da câmera
                ...
                -- Hover: para ao soltar o analógico
...
Rayfield:Notify({
    ...
    Content = "Fly integrado à Rayfield com sucesso!",
    Duration = 5,
    Image = 4483362458
})