...
-- Criar uma ScreenGui separada para garantir ZIndex e organização
...
TpFloatingBtn.Position = UDim2.new(0.8, 0, 0.7, 0)  -- canto inferior direito
...
TpFloatingBtn.Visible = false  -- controlado pelo Toggle
...
local hasMoved = false  -- para diferenciar toque de arraste
...
TpFloatingBtn.InputChanged:Connect(function(input)
    if isDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        if delta.Magnitude > 5 then  -- se moveu mais de 5 pixels, é arraste
            hasMoved = true
        end
...
tpFrame.Size = UDim2.new(0, 350, 0, 510)
tpFrame.Position = UDim2.new(0.5, -175, 0.5, -255)
tpFrame.BackgroundColor3 = Color3.fromRGB(15, 17, 25)  -- fundo escuro sutil
tpFrame.BackgroundTransparency = 0.1  -- apenas 10% de transparência, bem visível
...
-- Cantos arredondados
Instance.new("UICorner", tpFrame).CornerRadius = UDim.new(0, 16)

-- Sombra da janela
local winShadow = Instance.new("UIStroke", tpFrame)
winShadow.Thickness = 4
winShadow.Color = Color3.fromRGB(0, 0, 0)
winShadow.Transparency = 0.6
local winBorder = Instance.new("UIStroke", tpFrame)
winBorder.Thickness = 1
winBorder.Color = Color3.fromRGB(35, 42, 65)
...
titleFix.BackgroundColor3 = Color3.fromRGB(18, 20, 30)
...
-- Efeitos hover
local function hover(btn, base, hoverColor)
    btn.MouseEnter:Connect(function() TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = hoverColor}):Play() end)
    btn.MouseLeave:Connect(function() TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = base}):Play() end)
end
hover(minimizeBtn, Color3.fromRGB(30,35,50), Color3.fromRGB(50,55,70))
hover(closeBtn, Color3.fromRGB(180,40,40), Color3.fromRGB(220,60,60))
...
posLabel.Size = UDim2.new(1, -24, 0, 38)
posLabel.Position = UDim2.new(0, 12, 0, 46)
...
inputName.Size = UDim2.new(1, -24, 0, 46)
inputName.Position = UDim2.new(0, 12, 0, 92)
inputName.PlaceholderText = "Nome do Local"
inputName.PlaceholderColor3 = Color3.fromRGB(160, 160, 180)
...
saveBtn.Size = UDim2.new(1, -24, 0, 46)
...
saveStroke.Transparency = 0.6
hover(saveBtn, Color3.fromRGB(0,120,220), Color3.fromRGB(0,150,255))

-- Container de scroll
local container = Instance.new("ScrollingFrame", tpFrame)
container.Size = UDim2.new(1, -24, 0, 240)
container.Position = UDim2.new(0, 12, 0, 204)
...
clearBtn.Size = UDim2.new(1, -24, 0, 44)
clearBtn.Position = UDim2.new(0, 12, 0, 455)
...
hover(clearBtn, Color3.fromRGB(180,40,40), Color3.fromRGB(220,60,60))
...
-- Função toggleMenu (abre/fecha janela com animação)
function toggleMenu(show)
    if show then
        tpFrame.Visible = true
        tpFrame.Size = UDim2.new(0, 0, 0, 510)
        tpFrame.BackgroundTransparency = 1
        TweenService:Create(tpFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, 350, 0, 510),
            BackgroundTransparency = 0.1
        }):Play()
        -- Esconder botão flutuante enquanto janela está aberta
...
minimizeBtn.MouseButton1Click:Connect(function()
    toggleMenu(false)  -- só esconde a janela, botão reaparece
end)

closeBtn.MouseButton1Click:Connect(function()
    toggleMenu(false)
    teleportToggle:Set(false)  -- desliga o Toggle, sistema encerra
end)

-- Toggle principal da Rayfield
local teleportToggle = TeleportTab:CreateToggle({
    Name = "Ativar Sistema de Teleporte",
    CurrentValue = false,
    Callback = function(value)
        if value then
            -- Ativou: mostra botão flutuante
            if not tpFrame.Visible then
                animateBtn(TpFloatingBtn, true)
            end
        else
            -- Desativou: fecha tudo e esconde botão
            if tpFrame.Visible then
                tpFrame.Visible = false  -- força fechar sem animação para sumir logo
            end
            animateBtn(TpFloatingBtn, false)
        end
    end
...
Rayfield:Notify({
    ...
    Content = "Sistema de teleporte 100% funcional e modernizado!",
    Duration = 5,
    Image = 4483362458
})