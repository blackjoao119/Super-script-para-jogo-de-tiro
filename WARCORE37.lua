...
MovimentTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        if NoClipAtivo then
            ...
            -- Restaura imediatamente a colisão
            local char = Player.Character
            if char then
                for _, part in ipairs(char:GetDescendants()) do
                    if part:IsA("BasePart") then
...
--// =============================================
--// 🔥 SISTEMA DE TELEPORTE – LOCAIS FIXOS + DELETAR INDIVIDUAL
--// =============================================
...
-- Atualiza a lista visual (fixos primeiro, depois do usuário)
local function refreshContainer()
    -- Remove todos os elementos visuais do container
    for _, child in ipairs(container:GetChildren()) do
        child:Destroy()
    end

    -- 1. Locais Fixos (cor dourada, sem botão deletar)
    for i, loc in ipairs(FixedLocations) do
        local btn = Instance.new("TextButton", container)
        btn.Size = UDim2.new(1, -10, 0, 44)
        btn.Text = loc.name .. "\n" .. string.format("%.1f, %.1f, %.1f", loc.x, loc.y, loc.z)
        btn.BackgroundColor3 = Color3.fromRGB(255, 200, 0)  -- dourado
        btn.TextColor3 = Color3.fromRGB(30, 30, 30)         -- texto escuro para contraste
        ...
        -- Efeito hover (dourado mais claro)
    ...
    -- 2. Locais do Usuário (com botão X para deletar)
    for i, loc in ipairs(savedLocations) do
        ...
        itemFrame.LayoutOrder = #FixedLocations + i
        ...
        -- Índice local capturado corretamente
        local index = i
    ...
    -- Atualiza o CanvasSize para o scroll funcionar
    container.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 10)
end

-- Salvar local (usuário)
...
-- Limpar tudo (apenas locais do usuário)
clearBtn.MouseButton1Click:Connect(function()
    if #savedLocations == 0 then
        Rayfield:Notify({Title = "Aviso", Content = "Nenhum local do usuário para limpar.", Duration = 2, Image = 4483362458})
        return
    end
    savedLocations = {}
    refreshContainer()
    Rayfield:Notify({Title = "Limpo", Content = "Todos os seus locais foram removidos.", Duration = 2, Image = 4483362458})
end)

-- ========================
-- 2. FUNÇÕES DO TELEPORTE
-- ========================
...
-- Atualiza posição em tempo real e já exibe a lista de locais fixos
...
Rayfield:Notify({
    ...
    Content = "Teleporte atualizado: locais fixos e delete individual!",
    Duration = 5,
    Image = 4483362458
})